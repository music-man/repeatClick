...
{
    if (event == nil || [self isKindOfClass:NSClassFromString(@"UITabBarButton")]) {
        //UITextInput replaceRange:withText: will call
        [self dj_repeat_sendAction:action to:target forEvent:event];
        return;
    }
  
//    如果需要针对某个按钮设置防重复 可以把下边这个判断打开 只要设置了btnTag的值即可，默认是666可以自己更改
    ...
    if ([UIView dj_repeat_checkSafe]) {
        if (![NSStringFromSelector(action) hasPrefix:@"growingHook"]) {
            ...
            if (_lastDate == nil) {
                ...
                if ([[NSDate date] timeIntervalSinceDate:_lastDate]> interval){
                    _lastDate = [NSDate date];
                    [self dj_repeat_sendAction:action to:target forEvent:event];
                }else{
                    _lastDate = [NSDate date];
